...
import {
  ...
  Lock,
  ShieldCheck,
} from "lucide-react"; // Added Settings, ShieldCheck
...
interface LoginSelectionProps {
  ...
  rpcQrClientIdOverride: string; // NEW
  setRpcQrClientIdOverride: (id: string) => void; // NEW
  handleSetRpcQrClientIdOverride: (e?: React.FormEvent) => Promise<void>; // NEW
}
...
export const LoginSelection = ({
  ...
  onSwitchToToken,
  onSwitchToMaster,
  rpcQrClientIdOverride, // NEW
  setRpcQrClientIdOverride, // NEW
  handleSetRpcQrClientIdOverride, // NEW
}: LoginSelectionProps) => {
  const [showAdvancedSettings, setShowAdvancedSettings] = useState(false); // NEW
  ...
  return (
    ...
    >
      ...
      <div className="space-y-6">
        ...
        {/* NEW: Advanced Settings for Client ID Override */}
        <M3Card
          onClick={() => setShowAdvancedSettings(!showAdvancedSettings)}
          className="flex items-center justify-between group py-6 px-6 cursor-pointer"
        >
          <div className="flex items-center gap-4">
            <div className="p-3 rounded-m3-lg bg-m3-surfaceVariant/30 text-m3-onSurfaceVariant shadow-sm">
              <Settings className="w-6 h-6" />
            </div>
            <div>
              <h3 className="font-bold text-lg">Advanced Settings</h3>
              <p className="text-xs text-m3-onSurfaceVariant mt-1 uppercase tracking-widest font-bold">
                RPC/QR Client ID Override
              </p>
            </div>
          </div>
          <ChevronRight
            className={`w-5 h-5 text-m3-outline transition-transform ${
        ...
        {showAdvancedSettings && (
          <M3Card className="flex flex-col gap-6 p-6">
            <h3 className="font-bold text-lg">Client ID Override</h3>
            <div className="flex flex-col gap-2">
              ...
              <input
                type="text"
                required
                value={rpcQrClientIdOverride}
                onChange={(e) => setRpcQrClientIdOverride(e.target.value)}
                className="m3-input-filled shadow-inner"
                placeholder="e.g., 947330329678028831"
              />
            </div>
            <button
              onClick={handleSetRpcQrClientIdOverride}
              disabled={isLoading || !rpcQrClientIdOverride}
              className="m3-button-primary w-full shadow-lg"
            >
              Save RPC/QR Client ID
            </button>
          </M3Card>
        )}
      </div>
